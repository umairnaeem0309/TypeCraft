...
def _project_root() -> Path:
    """
    The project root when running from source (dev mode).

    This file lives at <project_root>/core/paths.py, so the project root
...
def resource_path(relative: str) -> Path:
    ...
    Args:
        relative: path relative to the project root, e.g. "assets/images/logo.png"
...
def writable_data_dir() -> Path:
    ...
    When running from source, this is a local ./_dev_data folder so dev
    runs never dirty the real project tree or get committed by accident.

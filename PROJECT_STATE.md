...
| Lesson JSON fallback does not warn the teacher | **RESOLVED** by TC-023 — log + NoticeBar warning |
...
- `typecraft/scenes/results.py` — Retry grey, Continue green, Leaderboard orange.
- `typecraft/ui/theme.py` — `LAYOUT_FOOTER_HEIGHT` (64), `LAYOUT_FOOTER_MARGIN` (20).
- `typecraft/scenes/lesson.py` — `TEXT_AREA` narrowed to 200–330; new `FOOTER_RECT` and
  `KEYBOARD_Y` **derived** from the footer constants and `KeyboardRenderer.size()`, so the
  5 px keyboard/footer overlap cannot recur; `_render_footer()` draws a band with a top rule;
  hint promoted from small to body size.
- `typecraft/ui/keyboard_renderer.py` — `ROW_OFFSETS` so rows stagger and the space bar
  (6.5 units, was 12 flush-left) sits under the letters; caption grammar fixed
  ("use either thumb", not "use your either thumb").
- `typecraft/scenes/teacher_dashboard.py` — confirmation now reads "Reset <name>'s data?".
- `typecraft/scenes/settings.py` — rebuilt as two 1000 px-wide cards with 60–64 px controls,
  a 560 px volume bar, live percentage, helper text, and a mute button that turns amber when
  muted.
- `tests/scenes/test_layout_regressions.py` — **new**, 17 tests.
- `TASKS.md`, `PROJECT_STATE.md`.

**Evidence:** 731 passing, 4 skipped, 0 failures. Verified by rendering Lesson, Settings and
Results to PNG and inspecting them — which is how two further faults (lopsided space bar,
caption grammar) were caught that the geometry assertions had passed.
